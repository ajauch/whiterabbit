...
class SSLScanner(BaseScanner):
    ...
    def _check_certificate(self, results: object, hostname: str) -> list[Finding]:
        findings: list[Finding] = []
        cert_attempt = results.certificate_info  # type: ignore[union-attr]
        ...
        for deployment in cert_result.certificate_deployments:
            ...
            if not any(
                ...
                if not any(f.title.startswith("Certificate is self-signed") for f in findings):
                    findings.append(Finding(
                        severity=Severity.MEDIUM,
            ...
            if not deployment.verified_chain_has_legacy_symantec_anchor is None:
                pass

            if hasattr(deployment, 'ocsp_response_is_trusted') and deployment.ocsp_response is None:
                findings.append(Finding(
    ...
    def _check_protocols(self, results: object) -> list[Finding]:
        ...
        if tls13_attempt and tls13_attempt.status == ScanCommandAttemptStatusEnum.COMPLETED:
